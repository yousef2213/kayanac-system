        ...
        <div class="container">
            <div class="col-print mx-auto text-center my-4">
                ...
                <div class="total">
                    ...
                    <div class="isTab3 d-none ">
                        <div class="d-flex justify-content-between px-4 align-items-center my-2">
                            <h6 class="tobacco_tax font-main"> 0 </h6>
                            <h6 class="font-main"> ضريبة التبغ </h6>
                        </div>
                    </div>
            ...
            <div class="text-center mb-5 mt-2">
                {{-- <button class="btn btn-primary px-4 printBtn d-none" onclick="youPrint()"> Print Shift </button> --}}
                {{-- <button class="btn btn-primary px-4 closeBtn" onclick="closeShify()"> Close Shift </button> --}}
            </div>
        </div>
    </div>
    </div>
    ...
    <script>
        ...
        if (window.parameters) {
            ...
            if (items) {
                Optimzation = [];
                console.log(items);
                // console.log(items.company.tobacco_tax);
                if (items.company.tobacco_tax == 1) {
                    document.querySelector('.isTab3').classList.remove('d-none');
                }
                let item;
                let itemHold;
                let taxRateTotal = 0;
                ...
                items.InvoicesDetails.forEach(option => {
                    ...
                    let old_item = document.querySelectorAll(`.input-qtn-${option.unit_id}-${option.item_id}`);
                    if (old_item.length != 0) {
                        let itemO = Optimzation.find(op => op.item_id == option.item_id && op.unit_id == option
                            .unit_id);
                        itemO.qtn += option.qtn;
                        document.querySelector(`.input-qtn-${option.unit_id}-${option.item_id}`).innerHTML = option
                            .qtn + +document.querySelector(`.input-qtn-${option.unit_id}-${option.item_id}`)
                            .innerHTML
                        document.querySelector(`.total-${option.unit_id}-${option.item_id}`).innerHTML = option
                            .total + +document.querySelector(`.total-${option.unit_id}-${option.item_id}`).innerHTML
                    } else {
                        ...
                        item = `
                            <tr class="">
                                ...
                                <td class="input-qtn-${option.unit_id}-${option.item_id}"> ${option.qtn} </td>
                                <td class="total-${option.unit_id}-${option.item_id}"> ${option.nettotal || 0} </td>
                            </tr>`
                ...
                let img = `<img class="rounded" width="100" src="{{ asset('comp/${items.company.logo}') }}" alt="">`;
                $('.logo').html(img)
                ...
                items.Orders.forEach(order => {
                    tobacco_tax += order.tobacco_tax || 0;
                });
                tax_source = tax_source * netTotal2 / 100;
                $('.tax_source').html(tax_source);
                $('.tobacco_tax').html(+tobacco_tax.toFixed(2));
