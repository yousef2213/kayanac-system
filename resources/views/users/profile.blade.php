            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="card">
                        <div class="image">
                            @if ($profile->photo)
                                <img class="card-img-top img-fluid roundend-circle mt-4" id="imgprofile"
                                    style="border-radius:50%;height:80px;width:80px;margin:auto;"
                                    src="{{ asset('storage/photos/1/users/'.$profile->photo) }}" alt="profile picture">
                            @else
                ...
                <div class="col-md-8">
                    <form class="border px-4 pt-2 pb-3" method="POST" action="{{ route('profile-update-admin', $profile->id) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input type="file" class="profilePhoto" onchange="handelChangePhotos()" name="photo" value={{$profile->photo}} id="">
                            </div>
                            @error('photo')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
...
@push('scripts')
    ...
    <script>
        ...
        const handelChangePhotos = () => {
            let files = document.querySelector('.profilePhoto').files[0];
