...
@section('main-content')
    ...
    <div class="card">
        ...
        <div class="card-body font-main">
            ...
            <form dir="rtl" method="post" action="{{ route('salesBill.update', $invocie->id) }}" autocomplete="off"
                ...
                <div class="row py-2 text-right">
                        ...
                        <select name="customerId" class="form-control chosen-select" tabindex="4" dir="rtl">
                            @foreach ($customers as $supplier)
                                <option value={{ $supplier->id }}
                                    {{ $invocie->customerId == $supplier->id ? 'selected' : '' }}> {{ $supplier->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select name="payment" class="form-control" tabindex="4" dir="rtl">
                            <option value="1" {{ $invocie->status == 1 ? 'selected' : '' }}> كاش </option>
                            ...
                            @foreach ($invoiceList as $invoiceItem)
                                ...
                                <tr class="Item-{{ $invoiceItem->id }}">
                                    ...
                                    <td class="px-0">
                                        ...
                                        <input type="number" disabled name="listUpdate[{{ $invoiceItem->id }}][total]"
                                    ...
                                    <td class="px-0">
                                        <input type="number" disabled name="listUpdate[{{ $invoiceItem->id }}][value]"
                                            onkeyup="handeltaxRate(event,'{{ $invoiceItem->id }}')"
                                            class="form-control rate-{{ $invoiceItem->id }}"
                                            value="{{ $invoiceItem->value }}" />
                                    </td>
                                    <td class="px-0">
                                        <input type="number" disabled name="listUpdate[{{ $invoiceItem->id }}][nettotal]"
                    ...
                    <table class="table table-border">
                        ...
                        <tbody class="text-center">
                            <tr>
                                ...
                                <td colspan="1" style="font-size: 15px" class="safe"> {{ $totalVoice }} </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td style="font-size: 15px" class="nettotal_inovice">
                                    {{ $totalVoice + $totalTax - ($totalVoice * $invocie->taxSource) / 100 }} </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
