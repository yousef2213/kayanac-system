...
@section('main-content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb justify-content-end font-main">
            ...
            <li class="breadcrumb-item"><a href="/erp/public/ItemsPurchases"> مرتجع مبيعات </a></li>
    ...
    <div class="card">
        ...
        <div class="card-body font-main">
            ...
            <form dir="rtl" method="post" action="{{ route('SallesReturned.update', $invocie->id) }}" autocomplete="off"
                ...
                <div class="row py-2 text-right">
                    ...
                    <div class="col-12 col-md-6 mx-auto">
                        ...
                        <input type="datetime-local" autocomplete="off" value="{{ $invocie->dateInvoice }}"
                        ...
                        <select name="customerId" class="form-control chosen-select" tabindex="4" dir="rtl">
                            @foreach ($customers as $supplier)
                                <option value={{ $supplier->id }}
                                    {{ $invocie->supplier == $supplier->id ? 'selected' : '' }}> {{ $supplier->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row py-2 text-right">
                    <div class="col-12 col-md-6">
                        ...
                        <select name="payment" class="form-control" tabindex="4" dir="rtl">
                            <option value="1" {{ $invocie->payment == '1' ? 'selected' : '' }}> كاش </option>
                            <option value="2" {{ $invocie->payment == '2' ? 'selected' : '' }}> اجل </option>
                        </select>
                    </div>
                        ...
                        <tbody id="bodys">
                            @foreach ($invoiceList as $purchaseitem)
                                <tr class="Item-{{ $purchaseitem->id }}">
                                    ...
                                    <td class="px-0">
                                        <input type="number" disabled name="listUpdate[{{ $purchaseitem->id }}][total]"
                                    ...
                                    <td class="px-0">
                                        <input type="number" disabled name="listUpdate[{{ $purchaseitem->id }}][value]"
